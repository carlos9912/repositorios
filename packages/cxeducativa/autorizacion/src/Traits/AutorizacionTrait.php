...
trait AutorizacionTrait
{
    ...
    {
        $permisos = [];
        static::$perfiles = (static::$perfiles  == []) ?  $this->perfiles : static::$perfiles;
        foreach (static::$perfiles as $perfil) {
    ...
    public function can($permiso, $arguments = [])
    {
        static::$perfiles = (static::$perfiles  == []) ?  $this->perfiles : static::$perfiles;
        foreach (static::$perfiles as $perfil) {
            if ($perfil->especial === EAcceso::index(EAcceso::No)->getId()) {
                return false;
            }
            
            if ($perfil->especial === EAcceso::index(EAcceso::Si)->getId()) {
                return true;
            }
        }
    ...
    {
        foreach ($this->perfiles as $role) {
            if ($role->especial === EAcceso::index(EAcceso::No)->getId()) {
                return false;
            }
            if ($role->especial === EAcceso::index(EAcceso::Si)->getId()) {
                return true;
            }
