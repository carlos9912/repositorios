...
class AsignaturaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $listVentas = DB::table('movimientos AS m')->leftJoin('rel_sucursal_movimiento AS r','r.codmovimiento','=','m.codmovimiento')->leftJoin('tarifas AS t','t.codtarifa','=','m.codtarifa')->leftJoin('sucursales AS s','s.codsucursal','=','m.cod_sucursal_envia')->leftJoin('sucursales AS sr','sr.codsucursal','=','m.cod_sucursal_retira');
        ...
        if(!blank($request->codsucursal)){
            $listVentas = $listVentas->where('m.cod_sucursal_envia','=',$request->codsucursal)->orWhere('m.cod_sucursal_retira','=',$request->codsucursal);
        }
        if(!blank($request->estado)&&$request->estado>0){
            $listVentas = $listVentas->where('m.estado','=',$request->estado);
        }
        $listVentas = $listVentas->orderBy('m.codmovimiento')->select('r.*','t.*','s.nombres AS nombreSucursal','sr.nombres AS nombreSucursalRetira', 's.ubicacion', 'sr.ubicacion as ubicacionRetira', 'm.*')->get();
...

    {
        ...
        $listMovimientosGenrales = [];
        $listMovimientosTipo = [];
        $listMovimientosSucursales = [];
        $listMovimientosSucursalesTipo = [];
        $comisionesPagadas = DB::table('rel_sucursal_movimiento as r')->where('tipo_movimiento', '=', 30)->whereBetween('r.fecha_movimiento',[
            $inicio,
            $fin
        ])->select(DB::raw('SUM(valor_real) as total'))->first();
        $listSQL = [];
        if(blank($request->codsucursal)){
            $listSQL = DB::table('rel_sucursal_movimiento as r')->where('tipo_movimiento', '<>', 30)->leftjoin('tarifas AS t', function ($join) {
            $join->on('t.codtarifa', '=', 'r.codtarifa')
                ->whereNotNull('r.codmovimiento');
            })->leftjoin('recargas AS re', function ($join) {
            $join->on('re.codrecarga', '=', 'r.codrecarga')
                ->whereNotNull('r.codrecarga');
            })->join('sucursales as s', 's.codsucursal','=','r.codsucursal')->whereBetween('r.fecha_movimiento',[
                $inicio,
                $fin
            ])->where('r.estado', '=', 1)->select('r.*','t.*','s.*', 're.codrecpaq')->get();
        }else{
            $listSQL = DB::table('rel_sucursal_movimiento as r')->where('tipo_movimiento', '<>', 30)->leftjoin('tarifas AS t', function ($join) {
            $join->on('t.codtarifa', '=', 'r.codtarifa')
                ->whereNotNull('r.codmovimiento');
            ...
            $join->on('re.codrecarga', '=', 'r.codrecarga')
                ...
                $inicio,
                $fin
            ])->where('r.codsucursal', '=', $request->codsucursal)->where('r.estado', '=', 1)->select('r.*','t.*','s.*', 're.codrecpaq')->get();
        }
        foreach ($listSQL as $movimiento) {
            $listMovimientosSucursales[$movimiento->codsucursal][$movimiento->fecha_movimiento] = $movimiento;
            $listMovimientosGenrales[$movimiento->fecha_movimiento] = $movimiento;
            $listMovimientosSucursalesTipo[$movimiento->codsucursal][$movimiento->tipo_movimiento][$movimiento->fecha_movimiento] = $movimiento;
            $listMovimientosTipo[$movimiento->tipo_movimiento][$movimiento->fecha_movimiento] = $movimiento;
        }
        


        //dd($listMovimientosGenrales);

        return view('asignaturas.reportes-estadisticos',[
            'comisionesPagadas' => blank($comisionesPagadas) ? 0 : $comisionesPagadas->total,
            'listG' => $listMovimientosGenrales,
            'listT' => $listMovimientosTipo,
            'listS' => $listMovimientosSucursales,
            'listST' => $listMovimientosSucursalesTipo,
            'mensaje' => "",
...
public function reportesRecargas(Request $request)
    {
        if(!blank(\Auth::user()->codempleado)){
            return redirect()->route('errors.empleado-403');
        }
        ...
        $listMovimientosSucursalesTipo = [];
        $listRecargas = [];
        $listRecargasDetalles = [];
        $listRecargasIndex = [];
        $listRecargasPaquetes = [];
        
        
        $paquetesRealizadas = DB::table('rel_sucursal_movimiento as r')->join('recargas as re', 're.codrecarga', '=', 'r.codrecarga')->join('recargas_paquetes as rp', 'rp.codigo', '=', 're.codrecpaq')->where('r.estado', '=', 1)->where('r.tipo_movimiento', '=', 4)->whereBetween('r.fecha_movimiento',[
            $inicio,
            $fin
        ])->get();
        $comisionesPagadas = DB::table('rel_sucursal_movimiento as r')->where('tipo_movimiento', '=', 30)->whereBetween('r.fecha_movimiento',[
        ...
        foreach($paquetesRealizadas as $mov){
            if($mov->codrecpaq!=11 && $mov->codrecpaq!=12 && $mov->codrecpaq!=13 && $mov->codrecpaq!=14 && $mov->codrecpaq!=15){
                if($mov->tipo==2){
                  $listRecargasIndex[$mov->codpadre] = Arr::exists($listRecargasIndex, $mov->codpadre) ? $listRecargasIndex[$mov->codpadre]+$mov->valor_real : $mov->valor_real;
                }else{
                  $listRecargasPaquetes[$mov->codigo] = Arr::exists($listRecargasPaquetes, $mov->codigo) ? $listRecargasPaquetes[$mov->codigo]+$mov->valor_real : $mov->valor_real;
                }
                if(Arr::exists($listRecargas, $mov->nombre)){
                    $listRecargas[$mov->nombre]+= $mov->valor_real;
                    $listRecargasDetalles[$mov->nombre][]= $mov;
                }else{
                    $listRecargas[$mov->nombre]= $mov->valor_real+0;
                    $listRecargasDetalles[$mov->nombre][]= $mov;
                }
            }
        ...
        $listSQL = [];
        if(blank($request->codsucursal)){
            $listSQL = DB::table('rel_sucursal_movimiento as r')->where('tipo_movimiento', '=', 4)->leftjoin('tarifas AS t', function ($join) {
            $join->on('t.codtarifa', '=', 'r.codtarifa')
                ->whereNotNull('r.codmovimiento');
            })->leftjoin('recargas AS re', function ($join) {
            $join->on('re.codrecarga', '=', 'r.codrecarga')
        ...
        //dd($listMovimientosGenrales);
        //dd($listRecargasIndex, array_sum($listRecargas),$listRecargas);
        //dd($listRecargasPaquetes, $listRecargasIndex);
        return view('asignaturas.reportes-recargas',[
            'comisionesPagadas' => blank($comisionesPagadas) ? 0 : $comisionesPagadas->total,
            'listG' => $listMovimientosGenrales,
            'listT' => $listMovimientosTipo,
            'listS' => $listMovimientosSucursales,
            'listST' => $listMovimientosSucursalesTipo,
            'mensaje' => "",
            'listOperadores' => $listRecargasPaquetes,
            'listPaquetes' => $listRecargasIndex,
            'listRecargas' => $listRecargas,
            'listProductos' => DB::table('productos as p')->join('categorias as c','c.id_categoria','=','p.codcategoria')->pluck(DB::raw("(c.nombre||' - '||p.nombre) as nombre"), 'p.codproducto'),
...

    {
        ...
        $sucursal = !blank($sucursal->codsucursal) ? $sucursal : Sucursal::find($request->codsucursal);
        if(!blank(\Auth::user()->codempleado)&&$sucursal->codsucursal!=session('sucursal')->codsucursal){
            return redirect()->route('errors.empleado-403');
        }
...

    {
        ...
        $listVentas = DB::table('sucursal_consignaciones as um')->join('sucursales as s','s.codsucursal','=','um.codsucursal');

        if(!blank($request->codsucursal)){

            $listVentas = $listVentas->where('s.codsucursal','=',$request->codsucursal);

        }

        if(!blank($request->inicio)&&!blank($request->fin)){

            $listVentas = $listVentas->whereBetween('um.fecha_registro',[
        ...
        return view('asignaturas.consignaciones',[
            'mensaje' => "",
            'listConsignaciones' => $listVentas->orderBy('um.estado', 'desc')->orderBy('um.fecha_registro', 'desc')->select( 's.*','um.*', 'um.fecha_registro AS fechaRegistro')->get() ,
            'listSucursales'=>DB::table('sucursales')->where('estado',1)->pluck(DB::raw("nombres||' - '||ubicacion as total"), 'codsucursal'),
            'menus' => Menu::listMenusHijos()
        ]);
